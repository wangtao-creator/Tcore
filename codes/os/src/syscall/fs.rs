...
use crate::task::{
    current_process, current_user_token, suspend_current_and_run_next, /* , print_core_info*/
};
use crate::timer::get_timeval;
use crate::{
    fs::{
        Dirent, FdSet, File, FileClass, FileDescripter, IoVec, IoVecs, Kstat, NewStat, NullZero,
        MNT_TABLE, TTY,
    },
    gdb_print, gdb_println,
    mm::{
        print_free_pages, translated_byte_buffer, translated_ref, translated_refmut,
...
pub fn sys_newfstatat(fd: isize, path: *const u8, buf: *mut u8, flag: u32) -> isize {
    ...
    if fd == AT_FDCWD {
        ...
        if let Some(file) = open(
            ...
            gdb_println!(
                SYSCALL_ENABLE,
                "syscall_fstatat(fd:{}, path = {:?}, buff addr = 0x{:X},  [size = {}]) ",
                fd,
                path,
                buf as usize,
                stat.st_size
...
pub fn sys_renameat2(
    ...
    if let Some(old_file_class) = get_file_discpt(old_dirfd, &oldpath, &inner, OpenFlags::RDWR) {
        match old_file_class {
            FileClass::File(oldfile) => {
                ...
                if let Some(new_file_class) = get_file_discpt(new_dirfd, &newpath, &inner, oflags) {
                    // copy
...
pub fn sys_readlinkat(dirfd: isize, pathname: *const u8, buf: *mut u8, bufsiz: usize) -> isize {
    if dirfd == AT_FDCWD {
        ...
        gdb_println!(
            SYSCALL_ENABLE,
            "sys_readlinkat(dirfd = {}, pathname = {}, *buf = 0x{:X}, bufsiz = {}) = {}",
            dirfd,
            path,
            buf as usize,
            bufsiz,
