...
pub fn update_user_clock() {
    let core_id: usize = get_core_id();
    PROCESSOR_LIST[core_id].acquire_inner_lock().update_user_clock();
}

// when trap into kernel, use this func to update kernel clock
pub fn update_kernel_clock() {
    let core_id: usize = get_core_id();
    PROCESSOR_LIST[core_id].acquire_inner_lock().update_kernel_clock();
}
...
pub fn get_user_runtime_usec() -> usize {
    let core_id: usize = get_core_id();
    return get_time_us() - PROCESSOR_LIST[core_id].acquire_inner_lock().get_user_clock();
}
...
pub fn get_kernel_runtime_usec() -> usize {
    let core_id: usize = get_core_id();
    return get_time_us() - PROCESSOR_LIST[core_id].acquire_inner_lock().get_kernel_clock();
}

pub fn schedule(switched_task_cx_ptr: *mut TaskContext) {
    let core_id: usize = get_core_id();
    let idle_task_cx_ptr = PROCESSOR_LIST[core_id].acquire_inner_lock().get_idle_task_cx_ptr();
    unsafe {
